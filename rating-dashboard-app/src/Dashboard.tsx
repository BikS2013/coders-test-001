...
import { ChevronDown, Calendar, ChevronLeft, ChevronRight } from 'lucide-react'; // Removed unused Menu
...
export default function Dashboard() {
  ...
  const handleTimePeriodSelect = (periodId: string) => {
    ...
    if (period) { // Add check for undefined period
  ...
  const handleRatingCategorySelect = (categoryId: string) => {
    if (categoryId === 'all') {
  ...
  const SimpleCalendar = ({ selectedDate, onDateSelect, onClose }: SimpleCalendarProps) => {
  ...
  const generateChartData = () => {
    ...
    const currentDate = new Date(startDate); // Use const as setDate modifies in place
